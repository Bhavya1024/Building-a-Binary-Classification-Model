...
# Step 1: create probability dataset

probability_X_train = np.array([[emoticon_train_pred[i][0],feat_train_pred[i][0],text_train_pred[i][0]] for i in range(training_data_count)])
probability_X_val = np.array([[emoticon_valid_pred[i][0], feat_val_pred[i][0], text_val_pred[i][0]] for i in range(validation_data_count)])
probability_X_test = np.array([[emoticon_test_pred[i][0], feat_test_pred[i][0], text_test_pred[i][0]] for i in range(test_data_count)])

y_train = train_emoticon_Y
y_val = val_emoticon_Y

# Step 2: using Standard scalar for standardizing the values
scalar2 = StandardScaler()
probability_X_train_scaled = scalar2.fit_transform(probability_X_train)
probability_X_val_scaled = scalar2.transform(probability_X_val)
probability_X_test_scaled = scalar2.transform(probability_X_test)

# Step 3: Training Support vector classifier on probability values
stacked_svc = SVC(kernel='rbf', random_state=42)
stacked_svc.fit(probability_X_train_scaled, y_train)

# Step 4: predicting classes for test data
